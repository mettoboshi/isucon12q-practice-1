...
execute "Analyze nginx access log with alp" do
   command "#{BIN_DIR}/alp json --sort avg -r -m '/api/player/player/[a-z0-9]+,/api/player/competition/[0-9a-z]+/ranking' -o count,method,uri,min,avg,max,sum --file #{NGINX_LOG} > #{NGINX_LOG_BASE_DIR}/nginx-alp-avg.log.#{TIMESTAMP}"
...
execute "Analyze nginx access log with alp" do
   command "#{BIN_DIR}/alp json --sort sum -r -m '/api/player/player/[a-z0-9]+,/api/player/competition/[0-9a-z]+/ranking' -o count,method,uri,min,avg,max,sum --file #{NGINX_LOG} > #{NGINX_LOG_BASE_DIR}/nginx-alp-sum.log.#{TIMESTAMP}"
